...
// 注意，readable bytes空间才是要服务端要发送的数据，writable
// bytes空间是从socket读来的数据存放的地方。
...
// write数据的，因此 writable bytes 指的是用于存放socket上可读数据的空间,
// readable bytes同理
...
public:
  /*static const
   * int可以在类里面初始化，是因为它既然是const的，那程序就不会再去试图初始化了。*/
  ...
  // 一直取到end位置.
  // 在解析请求行的时候从buffer中读取一行后要移动指针便于读取下一行
...
private:
  ...
  void makeSpace(size_t len) // 调整可写的空间
  {
     ...
     **/
    // 当len > xxx + writer的部分，即：能用来写的缓冲区大小 <
    // 我要写入的大小len，那么就要扩容了
    if (writableBytes() + prependableBytes() < len + kCheapPrepend) {
      buffer_.resize(writerIndex_ + len);
    } else // 如果能写的缓冲区大小 >=
           // 要写的len，那么说明要重新调整一下Buffer的两个游标了.
           // p-kC表示调整前prependable bytes - kCheapPrepend。
    {
      // p-kC(prependable减去kCheapPrepend)的长度与writable
      // bytes的和就是可写的长度，如果len<=可写的部分,说明还能len长的数据还能写的下，但是可写的空间是p-kC的长度与writable
      // bytes 共同构成，而且p-kC的长度与writable
      // bytes不连续，所以需要将readable
      // bytes向前移动p-kC个单位，使得可写部分空间是连续的 调整前：
      // +-------------------------+----------------------+---------------------+
      // |    prependable bytes    |    readable bytes    |    writable bytes |
      // | kCheapPrepend |  p-kC   |      (CONTENT)       | |
      // +-------------------------+----------------------+---------------------+
      // |               |         |                      | | 0        <=     8
      // readerIndex     <=     writerIndex             size 调整后：
      // +---------------+--------------------+---------------------------------+
      ...
      std::copy(
          begin() + readerIndex_,
          begin() +
              writerIndex_, // 把这一部分数据拷贝到begin+kCheapPrepend起始处
          begin() + kCheapPrepend);
      readerIndex_ = kCheapPrepend;
